...
def test_range_gate() -> None:
    gd = ROOT / "godot/src/app/range_controller.gd"
    cs = ROOT / "unity/Assets/Sable/Range/RangeLoop.cs"
    if cs.is_file():
        text = cs.read_text(encoding="utf-8")
        if "lifted" not in text:
            raise AssertionError("Range must gate fire on AimSample.lifted")
        if "AimSample" not in (ROOT / "unity/Assets/Sable/Aim/AimSample.cs").read_text(encoding="utf-8"):
            raise AssertionError("Unity must keep AimSample")
        return
    if not gd.is_file():
        raise AssertionError("no Range implementation")

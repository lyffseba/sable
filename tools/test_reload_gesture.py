...
def test_empty_landmark_clears_held() -> None:
    """Hand-leave / empty Worker result must drop reload hold, not coast it."""
    state = {
        "handLm": charger_reload(),
        "finHeld": True,
        "reloadHeld": True,
        "lastDetAt": 1000,
        "det": {"x": 12.0, "y": 34.0},
    ...
    if state["handLm"] is not None:
        _fail("empty landmarks must null handLm — do not feed a ghost plug")
    ...
    state["handLm"] = charger_reload()
    state["finHeld"] = True
    state["reloadHeld"] = True
    unusable = {0: (0.50, 0.85), 4: (0.28, 0.82)}
    ...
    plug = charger_reload()
    if not apply_mp_landmarks([plug], state) or state["handLm"] is not plug:
        _fail("successful apply must still set handLm")
    held, mag, reloaded = maybe_reload_edge(state["reloadHeld"], reload_gesture(plug), 2)
    if not reloaded or not held or mag != MAG_CAP:
        _fail("after empty-landmark clear, charger-plug rising edge must refill")
...
def test_client_peek_and_order() -> None:
    ...
    if "S.handLm = null" not in skin or "S.reloadHeld = false" not in skin:
        _fail("fallbackSkin clear path must stay")
    worker = _fn(src, "onHandsWorkerMsg")
    fail = worker[worker.find('msg.type === "fail"') :]
    if "S.handLm = null" not in fail or "S.reloadHeld = false" not in fail:

...
1. **OFFLINE** (60s gallery on the Yard) or **ONLINE** (HUD-on-Yard always-practice, camera armed without lock) → **WARM UP** (local 60s, skip calib/lock — the waiting Yard is already live) / **ENTER RANGE** (shared live Yard, skip calib/lock when already lifted). Bay booth is parked.

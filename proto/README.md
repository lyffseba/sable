...
2. Raise a hand at the lid cam. Point the index (aim). Product gun path (see `research/TRACKING.md`, `research/HAND_FUTURE.md`): shark-fin thumb fires (#86); index+middle charger-plug reloads (#87). Pinch is not a trigger. SEEKING until lock or Space (`forceGun`). Cam-deny / **T** is DESKTOP (OS cursor / pad HID) — **non-product emergency**, never the product story. HID click must not replace the shoot gesture in gun mode. Product GUN pad is menus only. No mouse mesh / sable-mouse path.
3. Four corners — point the index. Shark-fin (thumb UP) to capture.
...
| **Space** (hold) | Q4 forceGun escape (not a shot; never auto-desktop) |

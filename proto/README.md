...
Mode chip: `PAD` / `GUN` / `DESKTOP` / `SEEKING`. Gallery SableHUD chips: hangar `WAIT` / `READY` / `LIVE` from `S.hangar`, then `SCORE` / `ROUND` / `60s GALLERY`. Lift plays a quiet mint-tell chirp (`Mint. Lift.`).

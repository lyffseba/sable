...
| Aim filter    | << 0.2 ms                      |
| HID fire      | **< 8 ms** HID→hitscan (`?sableperf=1` / `window.SablePerf`). Peek UV → house sphere → `markHid`. Look (gun kick / muzzle world) and Shared Bay `reportSharedBayFire` / `reportSharedBayPose` / lobby poll stay fire-and-forget **after** `markHid` — never on the click. |

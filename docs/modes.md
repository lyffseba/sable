...
- Round clock is `RANGE_MS` (60 s) on the 128 Hz sim (`galleryOver` / `galleryLeftMs`). Offline / WARM UP close locally. match_live snaps `over` from the room — a `fire_tick` at or past the bell does not credit.

...
- Do not force calib/lock on WARM UP when the waiting Yard is already live. `lobbyWarmup` phase-preserves (`alreadyLifted` / `lobby` / `range`) — `play()` lock would `resetLockState` and disarm the gun `#68` armed. Cold path still `play("range")`.

...
- End state is **GALLERY CLEAR** (score / hits / accuracy / combo / 60 s round). match_live GALLERY CLEAR is the room bell, not a per-client `simMs()`. match_live ACCURACY is room hits / room shots — not a local `S.shots++`.

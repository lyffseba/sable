...
Vendored at `proto/vendor/mediapipe/` (`@mediapipe/tasks-vision@0.10.21` + Google float16/1 `hand_landmarker.task`). `proto/hands_worker.js` loads those files first; jsDelivr + Google model host remain a fallback if they are missing. Used only to read 21 hand landmarks. Not a game asset. There is no published lite `.task`.
...
SABLE's implementations in `native/cv_input/include/sable/one_euro.hpp`, `native/mojo/one_euro.mojo`, and `proto/hands.js` are original first-party code written from the published equations. They are not copies of a third-party source tree.

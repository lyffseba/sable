...
| **Bay 1v1** | Original booth, cover vs open middle, first to 5 | Boot **BAY** local; lobby **ENTER BAY** room-owned first-to-5 |
...
- **M9:** Shared Bay 1v1 — lobby **ENTER BAY** room-owns score / pose / `fire_ms` rewind. Boot **BAY** stays local one-click. Offline / WARM UP / ENTER RANGE stay. HID never waits on the lobby POST. **Done.** Soft-lock + merge stay with Sable.

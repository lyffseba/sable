...
Recommend **merge to main when CI green**. **`v0.20.0` stood.** Build tags after this gallery HUD tip. Thin SableHUD chips only — same fire verb, AimSample untouched, R6 128 Hz / Hands Worker / `fire_ms` honesty stay. Shared house and shared Bay stay a lazy lobby snapshot + fire-tick rewind (not a fake global 128 Hz friend sim). SablePort seams are identity-only. Bay is parked (code may stay; player chrome off — no boot **BAY** / lobby **ENTER BAY**). Next after this: R5 when a modeler is in Blender.
...
- **M4:** Bay as a second playlist (original booth; first to 5; one art bible). **Parked** — boot **BAY** + lobby **ENTER BAY** are off player chrome; code may stay. Yard is the sole active map. Offline / WARM UP stay local one-click. **`v0.12.0` stood.**
- **M5:** Salt House is a playable **gallery mode** (60 s score / round clock / GALLERY CLEAR). Offline one-click stays. WARM UP / ENTER RANGE stay; Bay stays parked. **Done.** **`v0.19.0` stood.**
...
- **M8:** Bay 1v1 on the R6 contract — local first-to-5 pose at 128 Hz, HID fire outside, `fire_ms` stamp, thin SableHUD chips. Offline / WARM UP stay one-click. Bay is never the only gun. **Done, then parked** — not player-facing.
- **M9:** Shared Bay 1v1 — room-owns score / pose / `fire_ms` rewind (engineering `lobbyStartBay` / `startBay`). **Parked** — lobby **ENTER BAY** / boot **BAY** are off player chrome. Offline / WARM UP / ENTER RANGE stay. HID never waits on the lobby POST. Soft-lock + merge stay with Sable. Post-M9 honesty: Shared Bay net never taxes HID. Lobby poll must not auto `play("bay")` from waiting `phase === "lobby"`. `?sableperf=1` p99 < 8 ms still holds. `tools/test_sableperf.py` / `tools/test_shared_bay.py` fail loud if `reportSharedBayFire` lands inside the probe.

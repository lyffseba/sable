...
`Space` is the Q4 force-GUN escape (`forceGun`) when the camera works but no hand — SEEKING until lock or Space, never auto-desktop. `T` forces desktop aim (OS cursor → UV) as hidden debug — not the Q4 product path.

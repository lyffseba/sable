...
- Waiting-Yard `lobby` is live for mint-tell — `afterLiftState` chirps on first lift, same hysteresis as range. Always-practice must not omit the tell until WARM UP / ENTER RANGE. `armPracticeDesktop` invokes `afterLiftState` after writing DESKTOP truth. `enterGame` invokes `afterLiftState` after `setPhase`.

`Space` is the Q4 force-GUN escape (`forceGun`) when the camera works but no hand — SEEKING until lock or Space, never auto-desktop. `T` forces desktop aim (OS cursor → UV) as hidden debug — not the Q4 product path. KeyT must not disarm DESKTOP on cam-deny waiting Yard (`lobby` / `wait_practice`) — re-arm via `armPracticeDesktop`. When `camReady`, KeyT debug toggle-off stays.

...
3. **Shared house / Bay (rewind, not a tick).** Lobby Range is closed-form pose at `elapsed_ms` plus fire-tick rewind (`tools/lobby.py` `_pose_at` / `hit`). The room seed owns kind / peek / velocity / `born_ms` (`_next_spawn_ms`) — a late first poll must not stamp birth. Sit plates use the same closed-form as local practice (`sit_pose_y` / `sitPoseY`: bob from life, then drop) — not a per-client phase that would split two friends. Flyers use the same closed-form (`flyer_pose` / `flyerPose`: `y0 + vy0*t - 0.5*g*t^2`) — not a local Euler step that would miss the rewind sphere. Gallery SCORE / combo / hits / shots live on that same rewind (`_gallery_hit` / `_gallery_miss` / `_gallery_note_shot`). ESC is the same miss for combo (`_note_escapes` / `_gallery_escape`) — a plate that leaves drops the book at the closed-form leave, not the observer; `applySharedSim` snaps then tells. ESC is not a shot. The room owns the 60 s bell (`over` when `elapsed_ms >= RANGE_MS`). A `fire_tick` at or past the bell does not credit and is not a shot. `applySharedSim` snaps the book, then closes. Do not locally end match_live from `simMs()`. Do not paint match_live ROUND remaining from local `simMs()` — snap `S.elapsedMs` from room `elapsed_ms`. Do not invent match_live ACCURACY from local `S.shots++` — snap `S.shots` from room `shots`. Do not locally zero combo before the room. Shared Bay is a last-committed pose mailbox plus the same `fire_ms` rewind (`start_bay` / `_bay_hit`). `fire_ms` is quantized to `SIM_HZ`. Clients poll a lazy snapshot. Inventing a 128 Hz friend loop would lie about how two tabs already share plates and first-to-5.

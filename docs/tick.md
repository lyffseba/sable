...
- **Offline one-click stays.** No warm-up tax. Fire at tick 0 is legal — the first plate is already on the pad. Waiting-Yard always-practice arms the camera fire-and-forget (`armPracticeCam`) — that arm is not a fire wait and must not route through `play()` lock.

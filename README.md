...
- **ONLINE**: Waiting arena is HUD-on-Yard always-practice (thin chips, live plates, camera armed without lock). **WARM UP** is one-click local 60s without leaving the room (RETURN TO LOBBY is one click). Host **ENTER RANGE** shares the live Yard without a calib/lock trap. Bay booth is parked — not a player mode.
